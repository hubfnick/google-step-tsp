...
def solve(cities):
    ...
    for i in range(max_iter):
        ...
        temp*=DECAY
        #sum of path=N-1
        a=random.randrange(N-1-2)
        b=random.randrange(a+2,N-1)

        ifcrossed,dist_increase=if_node_cross(a,b,cities,path_list)
        if(ifcrossed or dist_increase<0):
            swap_node(a,b,path_list)
            #print(path_list)
        else:
            if(random.random()<math.exp(-1*dist_increase/temp)):
                print(math.exp(-1*dist_increase/temp))
                swap_node(a,b,path_list)
                #print(path_list)
    print("sa end ")
